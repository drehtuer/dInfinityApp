...
committed** except this file and the properties template next to it:
`.gitignore` excludes `keystore/*`, and a leaked release key cannot be
un-leaked — it can only be rotated, which breaks upgrades for everyone who
...
- `keystore.properties` — paths, aliases and passwords for both, read by the
  build. Copy `keystore.properties.example` and fill it in.
