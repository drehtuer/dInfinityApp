...
- [ ] **Decide what gravity does while the phone is held.** This is the pooling, and it is a design decision rather than a defect. `GravityTracker` starts each shake at "straight down relative to the screen" and then integrates gyroscope rates with nothing to re-anchor it, and `ShakeDriver` keeps the last sample's direction for the rest of the roll — so a vigorous shake can leave *down* pointing sideways in the tray, and the dice pour into a wall and pack against it. Seen at `20d6`, `100d20` and `100d4`; a tap roll, where gravity stays straight down, spreads and stacks nothing across eight seeds (`JoltBridgeTest`). Three ways out, and they are not equivalent:
  - gravity stays straight down in the tray and only the hand's lateral force moves the dice — the tray stops being a tilted box, and "tilt the phone and the dice slide" goes out of `docs/physics-and-rendering.md`;
  - anchor the direction to `TYPE_GRAVITY` instead of integrating the gyroscope — honest about which way down really is, and a phone held upright then pours every die to the bottom wall, which is correct and probably unusable;
  - keep the tilt but clamp it, so a tray can lean without ever becoming a chute.
- [ ] Dice still stick together and do not tumble at `20d6` on the phone. Almost certainly the same cause as the item above — dice pressed into a wall by a sideways *down* slide rather than tumble — but it is unproven until gravity is settled
