...
- [ ] Dice picker row (`1h`): the rules are `core/notation`'s `DicePicker` and are tested — what is left is the row itself, tap adds, long-press removes, count badge on the die. The set dropdown (`4a`) waits on the installed-set registry (4.4); until there is a second set to choose, a chooser with one entry is furniture
