...
- [ ] **A d12 is the size of a plum.** `ShapeGeometry.boundingRadiusPerSize` holds each solid's circumradius **per edge length** — `√3/2` for a cube, `(√3/4)(1+√5)` for a dodecahedron — but it is multiplied by `DieMaterial.sizeMm`, which is documented as "the die's nominal size *across*" and which `DieMaterial.boundingRadiusMm` reads as a diameter (`sizeMm / 2`). The two disagree by a factor of 2.8 for a d12 and the cube only looks right because a cube's edge and its size across are the same number. Everything is *consistent* — the hull, the mesh and the capacity rule all use the one formula, so the physics and the picture agree — but they agree on a 45 mm d12. Decide which `sizeMm` means, fix the catalogue to match, and re-record the golden cases (`docs/tables.md`, `docs/dice-sets.md`)
